...
"""
Returning a single unsampled report (in Google Analytics), using the Google Analytics API.
...
def ins_reports(service, account_id, web_property_id, profile_id):
    ...
    # unsampledReports/insert#examples

    # This request creates an new unsampled report.
    try:
        results = service.management().unsampledReports().insert(
            accountId=account_id,
            webPropertyId=web_property_id,
            profileId=profile_id,
            body={
                'title': 'API-Teste Total Audience 2',
                'start-date': '2017-05-23',  # also can be used '15daysAgo'
                'end-date': '2017-05-30',  # also can be used 'today'
                'metrics': 'ga:users',
                'dimensions': 'ga:dimension104'
...
def get_list_unsampled_reports(service, account_id, web_property_id, profile):
    # Note: This code assumes you have an authorized Analytics service object.
    # See the Unsampled Reports Developers Guide for details.

    # Requests a list of all Unsampled Reports for the authorized user.
    try:
        return service.management().unsampledReports().list(accountId=account_id,
                                                            webPropertyId=web_property_id,
                                                            profileId=profile).execute()
...
def get_data_unsampled_reports(service, account_id, web_property_id, profile, ur_id):
    # Note: This code assumes you have an authorized Analytics service object.
    # See the Unsampled Reports Developer Guide for details.
    ...
    try:
        for ur in ur_id:
            report = service.management().unsampledReports().get(accountId=account_id,
                                                               webPropertyId=web_property_id,
                                                               profileId=profile,
                                                               unsampledReportId=ur).execute()

            print_data_unsampled_reports(report)
...
def main():
    ...
    # ur_id = print_results(ins_reports(service, account_id, web_property_id, profile))
    ur_id = print_list_unsampled_reports(get_list_unsampled_reports(service, account_id, web_property_id, profile))
    # print_data_unsampled_reports(get_data_unsampled_reports(service, account_id, web_property_id, profile, ur_id))
    get_data_unsampled_reports(service, account_id, web_property_id, profile, ur_id)
